...
do
  ...
  aws ssm put-parameter --profile csProd --region ap-southeast-1 --overwrite --type "String" \
    --name $key \
    --value $val
done

echo "Done. Pushed configurations to AWS parameter store in prod environment."
